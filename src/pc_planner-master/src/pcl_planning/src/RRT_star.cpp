...
int RRT_star::reconstruct(Vector3d p)
{
	...
	// cout << "111nears size: " << nears.size() << endl;
	...
	cout <<"--------------------------------------------------------------------------------------------------------"<<endl;
	std::cout<<"usrt.size() "<<usrt.size()<<std::endl;
	...
	for (size_t j = 1; j < usrt.size() ; j++)//最后一个即p_new.node 也被扔进去了
	{
		...
		// std::cout<<"temp.cost  "<<temp.cost<<std::endl;
	...
	// std::cout<<"222nears.size()"<<nears.size()<<std::endl;
	...
	for (size_t i = 0; i < nears.size(); i++)
	{
		...
		// std::cout<<"nsrt.size()  "<<nsrt.size()<<std::endl;
		...
		// std::cout<<"start_tree.size()  "<<start_tree.size()<<std::endl;
		// std::cout<<"goal_tree.size()  "<<goal_tree.size()<<std::endl;

		RRT_Node& ni = get_Node(nears[i]);
		// std::cout<<"dist "<<dist<<"  nears[i].first  "<<"ni.cost "<<ni.cost<<std::endl;
		if (dist < ni.cost)
		{
			n++;
			...
			ni.parent = tpnew;
			ni.cost = dist;
			ROS_ERROR("update_cost");
		std::cout<<"i  "<<i<<"  nears[i].first  "<<nears[i].first<<" nears[i].second "<<nears[i].second<<std::endl;
			update_cost(nears[i]);
			ROS_ERROR("maybe die there111");
		}
	}
...
vector<RRT_Node> RRT_star::refined_path(const PCTrajNode& start, const PCTrajNode& goal)
{
	...
	double start_end_dist=start.get_dist(goal);
	...
	// }
					...
					//d是rand的球的半径，单位是m
						ros::Time t_rrt_Star=ros::Time::now();
						...
						// while (1)
						// rrt_star_info<<"rrt size  "<<path.size()<<"  "<<"N_max_avg "<<kkk*PCTrajNode::config.rrt_star_param.N_max_avg<<" "<<"max_refine_num  "<<kkk*PCTrajNode::config.rrt_star_param.max_refine_num;
						bool first_time=true; 
						double times_progress=0.5;
						double node_progress=1;
						double max_number=z*PCTrajNode::config.rrt_star_param.max_refine_num;
						// if(max_number<100)
						// 	max_number=100;
						// double max_number=PCTrajNode::config.rrt_star_param.max_refine_num;

						// if(max_number>PCTrajNode::config.rrt_star_param.max_refine_num)
						// 	max_number=PCTrajNode::config.rrt_star_param.max_refine_num;
						...
						{	
							...
							// get_path();
							// if (refine_times > PCTrajNode::config.rrt_star_param.max_refine_num)
							get_path();
							visualizeTrees();
							
							if (refine_times > max_number)
							{
								break;
							}
							// if (refine_times > max_number/3)
							// {
							// 	d = PCTrajNode::config.rrt_star_param.path_r*path.size()*PCTrajNode::PR.d_nom/2;
							// }
							// if (refine_times > 2*max_number/3)
							// {
							// 	d = PCTrajNode::config.rrt_star_param.path_r*path.size()*PCTrajNode::PR.d_nom/4;
								
							// }


							// if (refine_times > PCTrajNode::config.rrt_star_param.max_refine_num/3 && first_time==true)
							// {
									
							// 		times_progress=double(refine_times)/double(PCTrajNode::config.rrt_star_param.max_refine_num);
							// 		node_progress=PCTrajNode::config.rrt_star_param.N_rewire/(kkk*PCTrajNode::config.rrt_star_param.N_max_avg);
							// 		if(node_progress<times_progress/10)
							// 		{
							// 				first_time=false;
							// 				break;
							// 		}
						...
						double t_rrt_Star_time =(ros::Time::now() - t_rrt_Star).toSec() * 1000;
						cout << "t_rrt_Star_time " << t_rrt_Star_time << endl;
						...
						// rrt_star_info<<"  half_times, "<<" node_progress "<<node_progress<<" times_progress "<<times_progress;
						// rrt_star_info<<"   refine nodes number:  "<<PCTrajNode::config.rrt_star_param.N_rewire<<"  refine_times  " << refine_times <<endl;
