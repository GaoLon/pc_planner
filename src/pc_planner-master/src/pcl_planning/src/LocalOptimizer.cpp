...
void LocalOptimizer::triggerCallBack(const geometry_msgs::PoseWithCovarianceStamped & msg)
{
	...
	double curve=0;
	file.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/with_cur_1/drive_on_pc_average_curvature.txt", ios::in);
    string data_line;
	// int count=0;
	...
	// 	count++;
	// 	curve+=stod( datas[0] );
	// 	cout<<"stod( datas[0] ) "<<stod( datas[0] )<<endl;
	// 	cout<<"curve "<<curve<<endl;
	// 	cout<<"avg curve "<<curve/double(count)<<endl;
    ...
    file.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/corrds_map1.txt", ios::in);
    outfile.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/drive_on_pc_traj.txt", std::ofstream::out);
    outfile.clear();
    plan_time.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/drive_on_pc_plan_time.txt", std::ofstream::out);
	plan_time.clear();
    access_time.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/drive_on_pc_access_time.txt", std::ofstream::out);
	traj_length.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/drive_on_pc_traj_length.txt", std::ofstream::out);
	traj_length.clear();
	rrt_star_info.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/drive_on_pc_rrt_star_info.txt", std::ofstream::out);
	avg_cur.open("/home/xulong/driving_on_pc_one_layer/driving_on_pc/data/drive_on_pc_average_curvature.txt", std::ofstream::out);
    avg_cur.clear();
	data_line;
    int n = 0;
...
//--------------
			...
			for (auto node : result_node)
		{
			...
			average_curvature+=abs(theta2-theta1)/dist;
			}
		...
		all_average_curvature+=average_curvature/(result_node.size()-1);
		traj_length_all+=all_length;

		access_time<<pre_time<<std::endl;
		traj_length<<all_length<<std::endl;
		avg_cur<<average_curvature/(result_node.size()-1)<<std::endl;
		if(result_node.size()==0)
...
{	
	...
	{	
		...
		vector<PCTrajNode> result_node;
		result_node=optimized_path(pseudoOdom, pcn);
		visualizeTrees();
		double length_tmp=0;
		int asd=0;
		int i=0;
		...
		int count=0;
			for (auto node : result_node)
		{
			// 			if(asd<result_node.size()-1){
			// PCTrajNode node1=result_node[asd];
			// PCTrajNode node2=result_node[asd+1];
			// double dist = (node1).get_dist(node2);
			// // cout<<"dist"<<dist<<endl;
			// length_tmp+=dist;
			// asd++;
		...
		cout<<"avg "<<average_curvature/count<<endl;
...
{//actually rrt connect
	...
	//ocost=inf是work的？
		...
		//如果这里p清除的话，会导致djkfailed的时候，也就是opath很有可能是只有一个，然后p又是跟着opath走，导致也只有一个，而不是failed之前的那个
		// for (size_t i = 1; i < opath.size() - 1; i++)
		// for (size_t i = 1; i < opath.size(); i++)//挂
		if(dijk_failed==false)
		{
			...
			//dijk如果失败的话就还是按照上一回的中心位置来，然后继续优化
					for (size_t i = 0; i < Graph.size(); i++)//挂
			{
...
double LocalOptimizer::get_op_cost(size_t index, int from, int to)
{
	...
	// cout << "from_node  " << from_node.T(0,3)<<" "<< from_node.T(1,3)<<" "<< from_node.T(2,3)<< endl;
	// cout << "to_node  " << to_node.T(0,3)<<" "<< to_node.T(1,3)<<" "<< to_node.T(2,3)<< endl;
	VectorXd par = PCTrajNode::get_connect(from_node, to_node);
