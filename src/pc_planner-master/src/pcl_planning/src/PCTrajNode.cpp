...
//-------------------------------------------------------------------------------------------------
					...
					if (fabs(thetaf) > 1.57)
					{
						// ROS_ERROR("			thetaf is over 90 degree");
						result.clear();
						return result;
					}
					...
					// {
						...
						for (double seg = 0.0; seg < srt[3]; seg+=PR.d_nom)
						{
							...
							T_seg.row(0)[1] = -sin(path_seg[2]);
							T_seg.row(1)[0] = sin(path_seg[2]);
							/*if (T_b.row(0)[3] < 0)//看来这里是徐隆本来准备整一下倒着的,最后没有整,选择直接return之
							{
								T_seg.row(0)[3] = -path_seg[0];
								T_seg.row(1)[3] = -path_seg[1];
							}
...
double PCTrajNode::calculate_rho(pcl::PointXYZI& p, pcl::PointCloud<pcl::PointXYZI>::Ptr kdmap, pcl::KdTreeFLANN<pcl::PointXYZI> kd)
{
	// if (p.intensity != -1)
	// {
	// 	if (p.intensity < 0)
